...
function MultiSelect(options) {
    ...
    this.createTemplate = function () {
        ...
        if (options.required) {
            labelEl.innerHTML = `${options.title } <span class="text-danger">*</span>`;
        }else{
            labelEl.innerHTML = `${options.title }`;
        }
    ...
    this.createSearchResult = function (fetchedData) {
        ...
        if (fetchedData.status  == 1) {
            ...
            keys.map((index) => {
                ...
                menuListItems += `
                    <li data-id="${itm.id}" id="badge__${itm.id}" data-title="${itm.title}"  class="list-group-item multi-select__result-item">${itm.title}</li>
                `;
            });
    ...
    this.handleOnItemClick = function () {
        ...
        for (let index = 0; index < menuItems.length; index++) {
            ...
            el.addEventListener('click', function (e) {
                e.preventDefault();
                selectBoxEl.querySelector('.multi-select__container').classList.remove('border-danger');
    ...
    this.isRequired = function () {
        selectBoxEl.querySelector('.multi-select__container').classList.add('border-danger');
    };
    ...
    this.handleBadgeClick = function () {
        ...
        for (let index = 0; index < badgeEls.length; index++) {
            ...
            el.addEventListener('click', function (e) {
                e.preventDefault();
                if (!options.api) {
                    console.log('#badge__'+this.getAttribute('data-id'));
                    document.querySelector('#badge__'+this.getAttribute('data-id')).classList.remove('d-none');
                }
                this.remove();
