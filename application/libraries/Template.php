...
class Template {
    ...
    'htmlentities', 'html_entity_decode', 'htmlspecialchars', 'htmlspecialchasp_decode',
    'urlencode', 'urldecode',
    // Date / time modifiers
     ...
     * @return XTemplate
     */
    public function load($file, $tpldir = '', $files = null, $mainblock = 'main', $autosetup = true) {
    ...
    public function restart ($file, $tpldir = '', $files = null, $mainblock = 'main', $autosetup = true, $tag_start = '{', $tag_end = '}') {
        ...
        // Kenneth Kalmer
        $this->tpldir = $tpldir;
        if (defined('DIR_TEMPLATE') && empty($this->tpldir)) {
            $this->tpldir = DIR_TEMPLATE;
        }
    ...
    public function script_trim ($content){
        $content = str_replace("\n",'',$content);
    ...
    public function str_trim ($content){
        $content = str_replace("\n",'',$content);
        $content = str_replace("\t",'',$content);
        $content = str_replace("\s\s",'',$content);
        $content = str_replace("\s",'',$content);
     ...
     */
    public function parse ($bname='main') {
        if (isset($this->preparsed_blocks[$bname])) {
            ...
            $this->_set_error("parse: blockname [$bname] does not exist; $this->filename");
            return;
    ...
    */
     ...
     * @return string
     */
    public function text ($bname = 'main') {
     ...
     */
    public function out ($bname='main',$type=false,$json=false) {
        if($type == false)
            $out = $this->text($bname);
        else
            $out = $bname;
        ...
        if ( VIEW_TRIM == 1 )
        echo $this->str_trim ($out);
        else
        echo $out;
    }

    public function _out ($bname,$type=false) {
     ...
     */
    public function reset_block ($bname) {
	...
	public function _new () {
		...
		$this->tag_start_delim = '';
		$this->tag_end_delim = '';

		$this->filecontents = '';
		unset($this->blocks);
		$this->blocks = array();
		unset($this->parsed_blocks);
		$this->parsed_blocks = array();
		unset($this->preparsed_blocks);
		$this->preparsed_blocks = array();
		unset($this->block_parse_order);
		$this->block_parse_order = array();
		unset($this->sub_blocks);
		$this->sub_blocks = array();
		unset( $this->vars);
		$this->vars = array();
		unset($this->filevars);
		$this->filevars = array();
		unset( $this->filevar_parent);
		$this->filevar_parent = array();
